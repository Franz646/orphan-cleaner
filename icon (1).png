...
class OrphanCleanerPanel extends HTMLElement {
  ...
  async _doScan() {
    ...
    try {
      ...
      this._state.scannedAt  = new Date().toLocaleTimeString("it-IT");
      ...
      this._log(`Scansione: ${data.total} totali, ${this._state.allOrphans.length} orfane (${ts_n} timestamp, ${de_n} entry rimosse, ${un_n} unavailable, ${he_n} euristiche).`, 'ok');
    } catch (e) {
      this._log("Errore scansione: " + e.message, "err");
    } finally {
  ...
  async _doDelete(ids) {
    ...
    this._log(`Eliminazione di ${ids.length} entità…`, "info");
    this._updateAll();
    ...
    try {
      ...
      data.deleted.forEach(id => {
        ...
        this._log(`✓ Cancellata: ${id}`, "ok");
      });
      (data.failed || []).forEach(id => this._log(`✗ Fallita: ${id}`, "err"));
      this._log(`Completato: ${data.deleted_count} eliminate, ${data.failed_count} fallite.`,
                data.failed_count > 0 ? "warn" : "ok");
    } catch (e) {
      this._log("Errore eliminazione: " + e.message, "err");
    } finally {
  ...
  _updateButtons() {
    ...
    if (scanBtn) scanBtn.textContent = s.scanning ? "Scansione…" : "Scansiona";
  }
  _setDisabled(id, val) {
    const el = this._shadow.getElementById(id);
    ...
    if (!items.length) {
      wrap.innerHTML = this._emptyHTML(
        this._state.total != null
          ? "Nessuna entità orfana trovata."
          : "Premi Scansiona per cercare le entità orfane."
      );
      return;
    }
  ...
  _tableHTML(items) {
    const rows = items.map(e => {
      ...
      const badge = e.method === "timestamp"
        ...
        : e.method === "dead_entry"
        ? `<span class="badge badge-dead">entry rimossa</span>`
  ...
  _showConfirm() {
    ...
    if (msg) msg.innerHTML =
      `Stai per eliminare <strong>${n} entità</strong>.<br>L'operazione è <strong>irreversibile</strong>. Procedere?`;
...
<div class="main">
  <div class="stat-row">
    ...
    <div class="stat s-warn">   <div class="s-label">Non disponibili</div><div class="s-val" id="s-heur">—</div></div>
    <div class="stat s-ok">     <div class="s-label">Selezionate</div>    <div class="s-val" id="s-sel">0</div></div>
  </div>

  <div class="toolbar">
    <button class="btn-primary" id="btn-scan">Scansiona</button>
    <button id="btn-selall" disabled>Seleziona tutto</button>
    <button id="btn-desel"  disabled>Deseleziona</button>
    <input type="text" id="filter" placeholder="Filtra per entity_id o piattaforma…" disabled>
    <button class="btn-danger" id="btn-del" disabled>Elimina selezionate</button>
  </div>
  ...
  <div class="card">
    ...
    <div id="tbl-wrap"><div class="empty"><p>Premi <strong>Scansiona</strong> per cercare le entità orfane.</p></div></div>
  </div>
  ...
  <div class="info-card">
    ...
    <div class="service-row">
      ...
      <span class="svc-desc">Scansiona e spara l'evento <code>orphan_cleaner_orphans_found</code> — utile nelle automazioni.</span>
    </div>
    <div class="service-row">
      <span class="svc-name">orphan_cleaner.delete_orphans</span>
      <span class="svc-desc">Elimina entità specifiche o tutte le orfane. Supporta <code>dry_run: true</code>.</span>
    </div>
  </div>
</div>
...
<div class="overlay-wrap" id="overlay">
  <div class="modal">
    ...
    <div class="modal-btns">
      ...
      <button class="btn-danger" id="btn-confirm">Elimina</button>
    </div>
  </div>
</div>
`;
  ...
  _attachListeners() {
    ...
    s.getElementById("filter").addEventListener("input", () => {
      ...
      s.getElementById("tbl-meta").textContent =
        q ? `${this._filtered().length} di ${this._state.allOrphans.length}` : "";
    });
