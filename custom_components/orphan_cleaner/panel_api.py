"""
HTTP endpoints for Orphan Entity Cleaner panel.
"""
...
class OrphanCleanerScanView(HomeAssistantView):
    """GET /api/orphan_cleaner/scan — returns orphan entities as JSON."""
    ...
    async def get(self, request: web.Request) -> web.Response:
        ...
        min_age    = config.get(CONF_MIN_AGE_HOURS, DEFAULT_MIN_AGE_HOURS)
        aggressive = config.get(CONF_AGGRESSIVE, DEFAULT_AGGRESSIVE)
        try:
            ...
            return web.Response(
                ...
                text=json.dumps({
                    "total": total, "orphans": [o.as_dict() for o in orphans],
                    "min_age": min_age, "aggressive": aggressive, "error": None,
                }),
            )
        except Exception as exc:
            _LOGGER.exception("Scan API error")
            return web.Response(status=500, content_type="application/json",
                                text=json.dumps({"error": str(exc), "orphans": [], "total": 0}))


class OrphanCleanerDeleteView(HomeAssistantView):
    """POST /api/orphan_cleaner/delete — deletes specified entities."""
...
def async_register_views(hass: HomeAssistant) -> None:
    ...
    _LOGGER.debug("Orphan Cleaner HTTP views registered")
